...
onMounted(async () => {
  try {
    ...
    console.log('User.picture= ', User.value.picture)
  ...
  <div :class="route.path === '/' ? 'container-login' : 'container'">
    ...
    <nav class="tab" v-if="route.path !== '/'">
      <div style="position: absolute; top: 20px; flex-direction: row; display: flex;">
        <img src="./images/icons/supplier.png" style="width: 60px; height: 60px;">
        <div style="flex-direction: column; padding-top: 20px; padding-left: 20px; color: white; font-weight: bold;">
          <label style=" margin-bottom: 20px;"> SMART</label>
          <label style=" margin-bottom: 20px;"> INVENTORY</label>
        </div>
      </div>
      ...
      <RouterLink to="/Dashboard" class="tab-box" :class="{ active: activeTab === 'dashboard' }"
        ...
        <svg :style="{ fill: activeTab === 'dashboard' ? 'white' : '#b2b2b2', width: '30px', height: '30px' }"
          viewBox="-5 -5 35 35">
          <path
        ...
        <label :style="{ color: activeTab === 'dashboard' ? 'white' : '#b2b2b2' }"> Dashboard</label>
      </RouterLink>

      <RouterLink to="/Product" class="tab-box" :class="{ active: activeTab === 'product' }"
        @click="handleClick('product')">
        <svg :style="{ fill: activeTab === 'product' ? 'white' : '#b2b2b2', width: '30px', height: '30px' }"
          viewBox="-5 -5 35 35">
        ...
        <label :style="{ color: activeTab === 'product' ? 'white' : '#b2b2b2' }"> Product</label>
      ...
      <RouterLink to="/Product/Add" class="tab-box" :class="{ active: activeTab === 'addproduct' }"
        ...
        <svg :style="{ fill: activeTab === 'addproduct' ? 'white' : '#b2b2b2', width: '30px', height: '30px' }"
          viewBox="0 0 25 25">
          <path xmlns="http://www.w3.org/2000/svg"
            d="M17,11H13V7a1,1,0,0,0-2,0v4H7a1,1,0,0,0,0,2h4v4a1,1,0,0,0,2,0V13h4a1,1,0,0,0,0-2Z" />
        </svg>
        <label :style="{ color: activeTab === 'addproduct' ? 'white' : '#b2b2b2' }"> Add Product</label>
      </RouterLink>
